...
describe('Image HTTP API', () => {
  ...
  it('should handle HTTP POST on /files/:bucket', done => {
    const upload = {
      ...
      attach: { image: `${__dirname}/fixtures/image.png` },
    };
  ...
  it('should throw when HTTP POST on /files/:bucket with no file', done => {
    ...
    testUpload(upload)
      ...
      .expect(400, (error, { body }) => {
        ...
        expect(body.errors.image.message).to.be.equal(
          'Path `image` is required.'
        );
        expect(body.errors.image.name).to.be.equal('ValidatorError');
        expect(body.errors.image.type).to.be.equal('required');
        expect(body.errors.image.kind).to.be.equal('required');
        expect(body.errors.image.path).to.be.equal('image');
        done(error, body);
      });
  });
