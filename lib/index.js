...
const multer = require('@lykmapipo/multer');
...
const bucketInfoFor = (bucket = 'fs') => {
  ...
  const info = lodash.find(lodash.values(Buckets), { bucketName: bucket }) || Buckets.File;

  // return bucket info copy
  return common.mergeObjects(info);
};
 ...
 */
const FileTypes = {
  File: { type: mongooseCommon.ObjectId, ref: 'File', autopopulate: AUTOPOPULATE_OPTIONS },
  Image: { type: mongooseCommon.ObjectId, ref: 'Image', autopopulate: AUTOPOPULATE_OPTIONS },
  Audio: { type: mongooseCommon.ObjectId, ref: 'Audio', autopopulate: AUTOPOPULATE_OPTIONS },
  Video: { type: mongooseCommon.ObjectId, ref: 'Video', autopopulate: AUTOPOPULATE_OPTIONS },
  Document: {
...
const createBuckets = () => {
  ...
  const files = mongooseGridfs.createBucket(Buckets.File);
  const images = mongooseGridfs.createBucket(Buckets.Image);
  const audios = mongooseGridfs.createBucket(Buckets.Audio);
  const videos = mongooseGridfs.createBucket(Buckets.Video);
  const documents = mongooseGridfs.createBucket(Buckets.Document);
...
const createModels = () => {
  ...
  const File = mongooseGridfs.createModel(Buckets.File, urlsFor(Buckets.File), actions);
  const Image = mongooseGridfs.createModel(Buckets.Image, urlsFor(Buckets.Image), actions);
  const Audio = mongooseGridfs.createModel(Buckets.Audio, urlsFor(Buckets.Audio), actions);
  const Video = mongooseGridfs.createModel(Buckets.Video, urlsFor(Buckets.Video), actions);
  const Document = mongooseGridfs.createModel(
    Buckets.Document,
...
const modelFor = (bucket = 'fs') => {
  ...
  const Model = lodash.get(models, modelName);
...
const bucketFor = (bucket = 'fs') => {
  ...
  const Bucket = lodash.get(buckets, bucketName) || buckets.files;
...
const uploadErrorFor = (...path) => {
  ...
  const paths = {};
  lodash.forEach(common.uniq([...path]), pathName => {
    paths[pathName] = {
      type: 'required',
      path: pathName,
      value: undefined,
  ...
  const error = mongooseCommon.validationErrorFor({ paths });
  return error;
};
...
/* declarations */
const router = new expressRestActions.Router({ version: API_VERSION });
...
router.get(
  ...
  expressRestActions.getFor({
    get: (options, done) => {
      const File = modelFor(lodash.get(options, 'filter.bucket'));
      return File.get(lodash.omit(options, 'filter.bucket'), done);
    },
  })
);
...
router.post(PATH_LIST, (request, response, next) => {
  ...
  upload(request, response, error => {
    ...
    // build response
    const file = new File(lodash.first(request.files));
...
router.get(
  ...
  expressRestActions.getByIdFor({
    getById: (options, done) => {
      const File = modelFor(lodash.get(options, 'filter.bucket'));
      return File.getById(lodash.omit(options, 'filter.bucket'), done);
    },
  })
);
...
router.patch(
  ...
  expressRestActions.patchFor({
    patch: (options, done) => {
      const File = modelFor(lodash.get(options, 'filter.bucket'));
      return File.patch(lodash.omit(options, 'filter.bucket'), done);
    },
  })
);
...
router.put(
  ...
  expressRestActions.putFor({
    put: (options, done) => {
      const File = modelFor(lodash.get(options, 'filter.bucket'));
      return File.put(lodash.omit(options, 'filter.bucket'), done);
    },
  })
);
...
router.delete(
  ...
  expressRestActions.deleteFor({
    ...
    del: (options, done) => {
      const File = modelFor(lodash.get(options, 'filter.bucket'));
      return File.unlink(lodash.get(options, '_id'), done);
    },
  })
);
...
exports.uploadErrorFor = uploadErrorFor;
exports.uploaderFor = uploaderFor;
